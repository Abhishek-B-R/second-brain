    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>Choose a destination folder for &quot;{website?.title}&quot;</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-2 p-3 bg-muted/50 rounded-lg">
            ...
            <div className="flex items-center space-x-1">
              {currentFolderId ? (
                <Folder
                  className="h-4 w-4"
                  style={{ color: findFolderById(folders, currentFolderId)?.color || "#3b82f6" }}
                />
              ) : (
          ...
          {selectedFolderId !== null && (
            <div className="flex items-center space-x-2 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <div className="text-sm text-blue-600 dark:text-blue-400">Moving to:</div>
              <div className="flex items-center space-x-1">
                <ArrowRight className="h-3 w-3 text-blue-500" />
                {selectedFolderId ? (
                  <Folder
                    className="h-4 w-4"
                    style={{ color: findFolderById(folders, selectedFolderId)?.color || "#3b82f6" }}
                  />
                ) : (
                  <Home className="h-4 w-4 text-blue-500" />
                )}
                <span className="text-sm font-medium text-blue-600 dark:text-blue-400">{selectedFolderName}</span>
              </div>
            </div>
          )}
                ...
                <div className="space-y-1">
                  ...
                  <div
                    className={cn(
                      ...
                      currentFolderId !== null && selectedFolderId !== null && "hover:bg-accent/50",
                    )}
